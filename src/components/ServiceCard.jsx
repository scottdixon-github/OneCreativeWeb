  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-full bg-surface-sunken px-3 py-1 text-xs font-medium text-content-muted">
          {service.startingPrice}
        </span>
      </div>

      <h3 className="mt-5 font-display text-lg font-bold text-content">{service.title}</h3>
      <p className="mt-1.5 text-sm text-content-muted">{service.tagline}</p>

      <ul className="mt-4 flex-1 space-y-2">
        {service.features.slice(0, 3).map((feature) => (
          <li key={feature} className="flex items-center gap-2 text-sm text-content-muted">
      ...
      <div className="mt-5 flex items-center gap-1.5 text-sm font-semibold text-brand transition-all duration-300 group-hover:gap-2.5">
        Learn More
